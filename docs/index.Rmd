...
piwi_genes_dat %>% 
  write_tsv("PIWI_pirna_biogenesis_reactome_mouse_human_Expression_Atlas.txt")

rm(datasets_atlass_scaled)
...
piwi_genes_dat %>% 
  filter(str_detect(organism, "sapie")) %>% 
  count(dataset, AtlasAssayGroup, cell_type, developmental_stage,
                         disease, organism_part, progenitor_cell_type, 
                         cell_line, genotype)
```
...
piwi_genes_dat_heatmap <- 
    ...
    add_tile(c(AtlasAssayGroup, dataset, organism)) 
```
